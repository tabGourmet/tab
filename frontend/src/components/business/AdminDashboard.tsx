...
export const AdminDashboard: React.FC = () => {
  ...
  // @ts-ignore
  const logoBusiness = new URL(
    "../../assets/logo-bussines.png",
    import.meta.url,
  ).href;
  const {
    products,
  ...
  const unseenSessionCount = activeSessions.filter(
    (s) => !seenSessionIds.current.has(s.id),
  ).length;
  ...
  const unseenOrderCount = pendingOrderSessions.filter(
    (s) => !seenOrderSessionIds.current.has(s.id),
  ).length;
  ...
  const handleTabChange = (tab: typeof activeTab) => {
    ...
    if (tab === "notifications") {
      ...
      pendingOrderSessions.forEach((s) =>
        seenOrderSessionIds.current.add(s.id),
      );
    }
  };
    ...
    {user ? `${user.businessName || user.email}` : 'TAB - Gestión del Negocio'}
      ...
      >
        <button
          ...
          className={`nav-button ${activeTab === "metrics" ? "active" : "inactive"}`}
        >
          📊 Métricas
        </button>
        <button
          onClick={() => handleTabChange("notifications")}
          className={`nav-button ${activeTab === "notifications" ? "active" : "inactive"}`}
        ...
        <button
          ...
          className={`nav-button ${activeTab === "tables" ? "active" : "inactive"}`}
          style={{ position: "relative" }}
        >
          🪑 Mesas Activas ({activeSessions.length})
          {unseenSessionCount > 0 && (
            <span style={badgeStyle}>{unseenSessionCount}</span>
          )}
        </button>
        <button
          onClick={() => handleTabChange("manage-tables")}
          className={`nav-button ${activeTab === "manage-tables" ? "active" : "inactive"}`}
        >
          ⚙️ Gestionar Mesas ({tables.length})
        </button>
        <button
          onClick={() => handleTabChange("products")}
          className={`nav-button ${activeTab === "products" ? "active" : "inactive"}`}
        >
          🍽️ Productos ({products.length})
        </button>
        <button
          onClick={() => handleTabChange("orders")}
          className={`nav-button ${activeTab === "orders" ? "active" : "inactive"}`}
          style={{ position: "relative" }}
        >
          🧾 Pedidos
          {unseenOrderCount > 0 && (
            <span style={badgeStyle}>{unseenOrderCount}</span>
          )}
        </button>
      </div>
      {/* Metrics Tab */}
        ...
        <div className="menu-section">
          {activeSessions.length === 0 ? (
            ...
            activeSessions.map((sess) => {
              const totalOrders = sess.orders.reduce(
                (acc, order) =>
                  acc +
                  order.items.reduce(
              ...
              const pendingCalls = sess.serviceCalls.filter(
                (c) => c.status === "pending",
              );
              ...
              return (
                ...
                >
                  ...
                  >
                    <p>
                      👥 {sess.consumers.length} persona(s):{" "}
                      {sess.consumers.map((c) => c.name).join(", ")}
                    </p>
                    <p>
                      🧾 {sess.orders.length} pedido(s) - Total: $
                    ...
                    {pendingCalls.length > 0 && (
                      ...
                      >
                        ⚠️ {pendingCalls.length} llamado(s) pendiente(s)
                      </p>
                    )}
                  </div>
                  {/* SUGERIDO POR V0 */}
                  <button
                    onClick={async () => {
                      if (
                        window.confirm(
                          `Liberar Mesa ${sess.tableId}? Esto cerrara la sesion actual.`,
                        )
                      ) {
      ...
      {activeTab === "manage-tables" && (
        <div className="menu-section">
          ...
          >
            ...
            <div
              className="inputAddTableResponsive"
              style={{
                display: "flex",
                gap: "var(--spacing-sm)",
                alignItems: "center",
          ...
          {/* Tables list */}
          <h3 style={{ marginBottom: "var(--spacing-md)" }}>
            Mesas Existentes ({tables.length})
          </h3>
          {tables.length === 0 ? (
            <p className="text-center text-muted">
              No hay mesas creadas. Agregá mesas arriba.
            </p>
          ) : (
            <div
              style={{
                    ...
                    {editingTableId === table.id ? (
                      ...
                      <>
                        ...
                        >
                          ...
                          <button
                            onClick={() => toggleTableEnabled(table.id)}
                            style={{
                              padding: "0.25rem 0.5rem",
                              background: "transparent",
                              border: "1px solid var(--color-border)",
      ...
      {activeTab === "products" && (
        <div className="menu-section">
          ...
          {products.map((product) => (
            <div
              className="menu-section-products-responsive"
              key={product.id}
              style={{
                display: "flex",
                alignItems: "center",
            ...
            >
              ...
              />
              <div style={{ flex: 1 }}>
                <p className="productNameResponsive" style={{ fontWeight: 600 }}>{product.name}</p>
                <p className="productCategoryResponsive"
                  style={{
                    fontSize: "0.75rem",
                    color: "var(--color-text-muted)",
                  }}
      ...
      {qrModalTable && (
        ...
        >
          ...
          >
            ...
            >
              {window.location.origin}/{restaurantSlug}/table/
              {qrModalTable.number}
            </p>
