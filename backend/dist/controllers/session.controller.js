    ...
    join = async (req, res, next) => {
        try {
            ...
            if (!tableId || !consumerName) {
                throw new error_handler_1.AppError('Table ID and Name are required', 400);
            }
            // Check table exists
            ...
            let session = await database_1.default.session.findFirst({
                where: {
                    tableId,
                    status: 'ACTIVE'
                }
            ...
            if (!session) {
                session = await database_1.default.session.create({
                    data: {
                        tableId,
                        status: 'ACTIVE'
                    }
    ...
    getCurrent = async (req, res, next) => {
        try {
            ...
            try {
                ...
                throw new error_handler_1.AppError('Invalid token', 401);
            }
            ...
            if (!session || session.status !== 'ACTIVE') {
                throw new error_handler_1.AppError('Session not active', 404);
            }
